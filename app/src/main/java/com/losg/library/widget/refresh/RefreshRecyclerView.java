...
public class RefreshRecyclerView extends DesignRefreshLayout implements View.OnClickListener, SwipeRefreshLayout.OnRefreshListener {
     ...
     * 默认加载的位置
     */
    private int DEFAULT_START_LOADING = 5;
    private int DEFAULT_PAGE_MIN      = 20;
    ...
    protected LoadingStatus mLoadingStatus;        //当前加载状态
    protected BaFooterView  mFooterView;             //上拉加载View
    protected boolean mCanLoading = true;           //是否可以加载
    ...
    private void initView() {
        ...
        addView(mRecycler, new ViewGroup.LayoutParams(-1, -1));
    ...
    public void defaultValue(int startSize, int pageDefault, int spanSize){
        DEFAULT_START_LOADING = startSize;
        DEFAULT_PAGE_MIN = pageDefault;
        mSpanSize = spanSize;
    ...
    private RecyclerView.OnScrollListener mOnScrollChangeListener = new RecyclerView.OnScrollListener() {
        ...
        public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
            ...
            if (layoutManager instanceof LinearLayoutManager) {
                ...
                //查询当前访问位置
                if (lastCompletelyVisibleItemPosition >= layoutManager.getItemCount() - 1 - loadingStart) {
    ...
    public void setLoadingAll(boolean allLoad) {
        if (allLoad) {
            ...
            if (mRecycler.getAdapter().getItemCount() < DEFAULT_PAGE_MIN / mSpanSize) {
